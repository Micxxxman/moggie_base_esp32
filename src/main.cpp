...
const int         EPARS                 = CEPARS;           // EEPROM space for parameters
const int         BOOTST_TIMEOUT        = CBOOTST_TIMEOUT;  // minutes
const int         NPARS_WEB             = 2;                // number of parameters to show on the bootstrap page
// The actual parameter and dictionary objects to hold parameter values

Dictionary        d;                                        // dictionary for parameters
ParametersEEPROM  p(TOKEN, d, 0, EPARS);     
// #define USE_SOFT_AP // Uncomment if you want to enforce using Soft AP method instead of BLE


SemaphoreHandle_t     semRadio = NULL;
...
void prov_event_handler(void* user_data, wifi_prov_cb_event_t event, voide*event_data)
{
   switch(event){
    case WIFI_PROV_INIT:
    ...
    case WIFI_PROV_START:
        ESP_LOGI(TAG, "Provisioning strarted");
        break;
    case WIFI_PROV_CRED_RECV:{
         wifi_sta_config_t *cfg = (wifi_sta_config_t *)prov_event->event_data;

         String ssid = String((char *)cfg->ssid);
         String password = String((char *)cfg->password);

         //Log.verbose("Received Wi-Fi credentials ssid=%S password=%S" CR, ssid, password);
